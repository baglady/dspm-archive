// bridge-server.js — many phones/web clients -> one Octatrack over MIDI.
//
// The MIDI analogue of dspm-archive's OSC bridge. It:
...
//      the Octatrack's MIDI IN), so N clients never fight over the port.
//   3. Translates each {channel, value} message into a MIDI CC on the right
//      track channel, scaling the normalised 0..1 value to 0..127 (bipolar
//      params centre at 64).
//   4. Aggregates continuous controls across clients (mean) and edge-fires
//      toggles, at a fixed tick rate, so the OT only ever sees a sane CC stream.
...
//   BRIDGE_WS_PORT     8082   HTTP + WebSocket port (PWA + control socket)
//   MIDI_PORT_NAME     ''     substring match for the MIDI out port; '' = first
//   MIDI_VIRTUAL       0      1 = open a virtual port named "Octatrack Bridge"
//   BRIDGE_TICK_MS     25     CC flush interval (40 Hz)
//   AGG                mean   mean | last  (continuous aggregation across clients)
//   AUDIO_CHANNELS     1,2,3,4,5,6,7,8   per audio-track MIDI channel
//   AUTO_CHANNEL       9      AUTO/global MIDI channel
...
//     The output port is reopenable at runtime (POST /ports) so you can plug in
//     an interface and pick it from the UI without a restart. OT is output-only.
let easymidi = null;
...
function openOutput(want) {
  ...
  if (virtual) {
    try { midiOut = new easymidi.Output('Octatrack Bridge', true); midiOutName = '(virtual)'; midiBackend = 'easymidi(virtual)'; }
...
const PORT = parseInt(process.env.BRIDGE_WS_PORT || '8082', 10);
...
const AUDIO_CHANNELS = (process.env.AUDIO_CHANNELS || MAP.defaults.audioChannels.join(','))
...
fs.writeFileSync(path.join(sessionDir, 'manifest.json'), JSON.stringify({
  spec: 'octatrack-midi-control/session/v1',
  sessionId, startedAt: new Date(startedAt).toISOString(),
  midiBackend, audioChannels: AUDIO_CHANNELS, autoChannel: AUTO_CHANNEL,
}, null, 2));
...
function channelToMidi(channel) {
  ...
  if (!def) return null;
  let ch;
  if (def.scope === 'audio') ch = AUDIO_CHANNELS[def.track - 1];
  else ch = AUTO_CHANNEL;                       // auto + global
  return { midiChannel: ch - 1, cc: def.cc, def };
}

// normalised 0..1 -> 0..127, honouring bipolar centre
function scale(value, def) {
  let v = Math.max(0, Math.min(1, value));
  const out = Math.round(v * 127);
  return out;
}

// Guarded send: a transient hardware error (port unplugged, the H4MIDI WC router
// momentarily reconfigured) must not crash the bridge. Log rate-limited + drop.
let lastSendErr = 0;
...
setInterval(() => {
  // continuous params: aggregate across clients
  for (const [channel, a] of acc) {
    const m = channelToMidi(channel);
    if (!m) continue;
    let norm = AGG === 'last' ? a.last : (a.count ? a.sum / a.count : a.last);
    if (crowd.enabled) sendCC(m.midiChannel, m.cc, scale(norm, m.def), channel);
  }
  acc.clear();
  // toggles: edge-fire whatever queued
...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname === '/channels') {            // UI introspection
    ...
    return res.end(JSON.stringify({
      audioChannels: AUDIO_CHANNELS, autoChannel: AUTO_CHANNEL,
      params: MAP.AUDIO_TRACK_PARAMS, globals: MAP.GLOBAL_PARAMS,
      midiTracks: MAP.MIDI_TRACK_CONTROLS, fxLegends: MAP.FX_LEGENDS,
    }));
  }
  ...
  if (u.pathname === '/admin/crowd') {         // kill-switch / dimmer
    ...
    res.writeHead(200, { 'content-type': 'application/json' });
    return res.end(JSON.stringify(crowd));
  }
...
server.listen(PORT, () => {
  console.log(`[bridge] octatrack-midi-control on :${PORT}  (MIDI: ${midiBackend})`);
  console.log(`[bridge] audio channels ${AUDIO_CHANNELS.join(',')}, AUTO ch ${AUTO_CHANNEL}`);
