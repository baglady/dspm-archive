-- dspm_archive v0.12.0+dspm
-- six-speed six-voice looper (schollz/barcode), patched for dspm-archive:
--   * osc.event handler -- /param/<id> generic forward, /barcode/<global>
--     underscore paths for space-containing param ids, /barcode/vN/<param>
--     for the six per-voice bias/lfo controls that live in voice[i] tables
--     and are NOT reachable through params (this is the same patch already
--     verified working against the installed norns OS in norns_project --
--     do not change the OSC paths below without updating the bridge and
--     any TouchOSC layouts that target them)
--   * perf_logger -- every param change (via the wrapped action below) and
--     every voice[i]/softcut transport event below gets written to a
--     timestamped JSONL file for the session-capture bundle
--
...
-- osc.event: verified working against the installed norns OS in
-- norns_project. Three tiers, checked in order:
--   1. generic /param/<id> -> params:set(id, value), covers everything
--      genuinely addressable as a param (filter cutoff/reso, recording,
--      clear, reverse, quantize, slew times, levels, etc). reverse/quantize
--      are 2-option params (1=off, 2=on); our controllers send 0/1, so
--      shift up by 1 for those two specifically.
--   2. /barcode/<global> underscore paths, for the few global param ids
--      that contain spaces and so can't be used literally as an OSC
--      address (rec level, pre level, the three slew times).
--   3. /barcode/vN/<param>, for the six per-voice bias/lfo controls that
--      live in the voice[i] tables rather than params -- these are the
--      only writes here that perf_logger needs to record explicitly,
...
osc.event = function(path, args, from)
  ...
  if path == "/barcode/rec_level" then
    params:set("rec level", v)
    return
  elseif path == "/barcode/pre_level" then
    params:set("pre level", v)
    return
  elseif path == "/barcode/rate_slew" then
    params:set("rate slew time", util.linlin(0,1,0,30,v))
    return
  elseif path == "/barcode/pan_slew" then
    params:set("pan slew time", util.linlin(0,1,0,30,v))
    return
  elseif path == "/barcode/level_slew" then
    params:set("level slew time", util.linlin(0,1,0,30,v))
    return
  elseif path == "/barcode/output_level" then
    -- master output level: this is state.level, set directly by E1 in
    -- enc() below -- it has no params entry at all, so unlike everything
    -- else in this tier-2 block it bypasses params:set entirely and needs
    -- its own explicit log line (the params hook never sees this write).
    state.level = util.clamp(v, 0, 1)
    logger.write("state_param", {id = "output_level", value = state.level})
    return
  end

  local voice_n, param = string.match(path, "/barcode/v(%d)/(%a+)")
  if voice_n == nil then return end
  local i = tonumber(voice_n)
  if i < 1 or i > 6 then return end

  if param == "level" then
    voice[i].level.adj = util.linlin(0,1,-2,2,v)
  elseif param == "pan" then
    voice[i].pan.adj = util.linlin(0,1,-2,2,v)
  elseif param == "rate" then
    voice[i].rate.adj = util.linlin(0,1,-const_num_rates,const_num_rates,v)
  elseif param == "direction" then
    voice[i].sign.adj = util.linlin(0,1,-2,2,v)
  elseif param == "start" then
    voice[i].ls.adj = util.linlin(0,1,-state.buffer_size[state.buffer],0,v)
  elseif param == "endpos" then
    voice[i].le.adj = util.linlin(0,1,0,state.buffer_size[state.buffer],v)
  elseif param == "level_lfo" then
    voice[i].level.lfo_period = util.linlin(0,1,1,50,v)
  elseif param == "pan_lfo" then
    voice[i].pan.lfo_period = util.linlin(0,1,1,50,v)
  elseif param == "rate_lfo" then
    voice[i].rate.lfo_period = util.linlin(0,1,1,50,v)
  elseif param == "direction_lfo" then
    voice[i].sign.lfo_period = util.linlin(0,1,1,50,v)
  elseif param == "startend_lfo" then
    voice[i].ls.lfo_period = util.linlin(0,1,1,50,v)
    voice[i].le.lfo_period = util.linlin(0,1,1,50,v)
  else
    return
  end
  logger.write("voice_param", {voice = i, param = param, value = v})
end
...
function undo()
  if state.recording==0 then
    ...
    logger.write("softcut", {event="undo", buffer=state.buffer})
    clock.run(function()
...
function start_recording()
  ...
  softcut.rec(1,1)
  logger.write("softcut", {event="rec_start", buffer=state.buffer})
...
function stop_recording()
  ...
  softcut.rec(1,0)
  logger.write("softcut", {event="rec_stop", buffer=state.buffer, recordingtime=state.recordingtime})
end
...
function key(n,z)
  ...
  elseif n==2 and z==1 and state.shift==1 then
    ...
    update_buffer()
    logger.write("softcut", {event="buffer_switch", buffer=state.buffer})
    clock.run(function()
...
function clear_recording()
  ...
  logger.write("softcut", {event="clear", buffer=state.buffer})
  clock.run(function()
